...
GPU = "time ffmpeg -y -hwaccel_device {hwaccel_device} \
...
rate_720 = "-b:a 128k -minrate 1000k -b:v 3000k -maxrate 5000k -bufsize 6M "
rate_1080 = "-b:a 192k -minrate 5M -b:v 8M -maxrate 10M -bufsize 12M "
...
def get_cmd_gpu(format, codec, height, file):
    ...
    if height >= 1080:
        ffmpeg_cmd = ffmpeg_cmd + scale_gpu.format(height=1080)
        if format == "m3u8":
            ffmpeg_cmd = (
                ffmpeg_cmd
                + SUBTIME
            ...
            ffmpeg_cmd = (
                ffmpeg_cmd
                + SUBTIME
                + end_1080_mp4.format(output_dir=VIDEOS_OUTPUT_DIR, output=filename)
            )
        ...
        if format == "m3u8":
            ...
            add_info_video(
                "encode_video",
                {
                    "encoding_format": "video/mp4",
                    "rendition": "1920x1080",
                    "filename": "1080p_{output}.mp4".format(output=filename),
                },
                True
            )
    return ffmpeg_cmd
